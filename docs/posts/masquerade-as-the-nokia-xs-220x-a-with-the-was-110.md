...
SFP+ ONT stick) on an XGS-PON network. Verified on TDS Telecom with the 8311
community firmware v2.8.3, behind a UniFi gateway.
...
| 8311 setting          | Value                        | Source                         |
|-----------------------|------------------------------|--------------------------------|
| `8311_gpon_sn`        | `ALCLxxxxxxxx`               | S/N on the label               |
| `8311_equipment_id`   | `BVMN300DRAXS220XA`          | CLEI code `BVMN300DRA` + model  |
...
3. On first association the OLT runs an OMCI software-image download and rewrites
   `8311_sw_verA`/`B` to the value it expects. The 8311 firmware persists the string
   to the environment rather than flashing anything. Allow ~10-15 minutes.
